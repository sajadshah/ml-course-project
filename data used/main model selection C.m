...
accuracyOverFolds = zeros(k,length(cValues));
fmeasureOverFolds = zeros(k,length(cValues));
for i=1:k
    b = bounds(i);
    e = bounds(i+1);
    if i==1
    ...
    for j=1:length(cValues)
        svmOptions = ['-s 0 -t 2 -g 0.5 -c ',num2str(cValues(j))];
...
info=[cValues', accuracy' ,fmeasure'];