...
df = {'cityblock','chebychev','correlation','cosine','euclidean','hamming','minkowski','seuclidean','spearman',gk};

accuracyOverFolds = zeros(k,size(df,2));
fmeasureOverFolds = zeros(k,size(df,2));
for i=1:k
    b = bounds(i);
    e = bounds(i+1);
    if i==1
